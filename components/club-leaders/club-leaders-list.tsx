...
export function ClubLeadersList() {
  const [regions, setRegions] = useState<ClubLeader[]>([])
  ...
  useEffect(() => {
    const fetchRegions = async () => {
      try {
        ...
        const RANGE = 'Clubs!A2:K' // Adjust range based on your sheet structure
        const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${RANGE}?key=${API_KEY}`

        const response = await fetch(url)
        if (!response.ok) throw new Error('Failed to fetch club leaders')
        
        const data = await response.json()
        
        const transformedRegions: ClubLeader[] = data.values.map((row: any[]) => ({
          name: row[0] || '',
          leader: {
            name: row[1] || '',
            image: row[4] || '',
        ...
        setError(err instanceof Error ? err.message : 'Failed to fetch club leaders')
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className="min-h-screen bg-gradient-to-b from-black-900 to-gray-800 py-12 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <div className="bg-red-500/10 text-red-500 p-4 rounded-xl inline-block">
            <h3 className="font-semibold">Error Loading Club Leaders</h3>
            <p>{error}</p>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-black-900 to-gray-800 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold text-white mb-4">Club Leaders</h2>
          <p className="text-gray-400 max-w-2xl mx-auto">
            Meet our dedicated club coordinators who help build and nurture our community across different locations
          </p>
        </div>
